...
export default function SellerDashboard() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', category: 'Metal', condition: 'Used', price: '', quantity: '', description: '', image: '' });
      setShowForm(false);
  ...
  const handleOrderAction = async (orderId: number, action: 'confirm' | 'reject') => {
    try {
      await api.put(`/orders/${orderId}`, { status: action === 'confirm' ? 'confirmed' : 'rejected' });
  ...
  const stats = {
    ...
    totalRevenue: orders.filter(o => o.status === 'approved').reduce((sum, o) => sum + parseFloat(o.total_price), 0)
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50">
      <DashboardNav role="seller" />
